<template>
  <div>
    <section class="hero is-primary">
      <div class="hero-body">
        <div class="container">
          <h1 class="title has-text-centered-desktop">User Recommendations</h1>
          <h2 class="subtitle has-text-centered-desktop">
            <span
              class="has-text-grey-dark is-size-6"
            >Looking for a game that's similar to what you've played?</span>
          </h2>
        </div>
      </div>
    </section>
      ...
      <div class="container">
        ...
        <div v-else-if="recommendations.length > 0">
          <article class="message">
            <div class="message-body">Currently, there are
              <strong>{{ total }}</strong> recommendations in the database.
              <strong>
                <router-link :to="{name: 'CreateRecommendation'}">Write a Recommendation</router-link>
              </strong>
            </div>
          </article>
        ...
        <div class="columns is-multiline is-centered" v-else>
          <div class="column is-12">
            ...
            >Currently, there are no recommendations in the database. Be the first to write one!
              <router-link :to="{name: 'CreateRecommendation'}">Write a Recommendation</router-link>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getRecommendations(page = 1) {
      axios
        .get("/api/recommendations?page=" + page)
        .then(response => {

    ...
    <section class="hero is-primary">
      <div class="hero-body">
        <div class="container">
          ...
          >{{ this.recommendation.second_release.alternate_title ? this.recommendation.second_release.alternate_title : this.recommendation.second_game.title }} Recommendation</h1>
          <h2 class="subtitle has-text-centered-desktop">
            <span class="tag is-light">
              <a
                :href="'/games?platforms=[' + this.recommendation.second_release.platform.id + ']'"
                :title="this.recommendation.second_release.platform.name"
              >{{ this.recommendation.second_release.platform.name }}</a>
            </span>
            <span class="has-text-grey-dark is-size-6">Recommended by
              <router-link
                :to="{name: 'User', params: { id: this.recommendation.user.id }}"
                class="dotted-border"
              >{{ this.recommendation.user.username }}</router-link>
    ...
    <section class="section">
      <div class="container">
        ...
        <div v-if="editMode">
          ...
          <recommendation-form :recommendation="recommendation" @submit="updateRecommendation"></recommendation-form>
        </div>
          ...
          <div class="columns">
              ...
              <div class="recomendation-image is-pulled-right" :data-tooltip="getSecondGameTitle">
                <b-tooltip :label="getSecondGameTitle" position="is-bottom">
                  <div class="card image imageFade text-container">
                    ...
                    >
                      <img
                        v-lazy="this.$store.state.cdnURL + 'games/icons/' + recommendation.second_game.icon"
                        :alt="getSecondGameTitle"
                        class="image is-128x128"
                      >
                    </router-link>
                  </div>
            ...
            <div class="column">
              <h2 class="subtitle">
                "If you liked
                <router-link
                  :to="{name: 'Game', params: { id: this.recommendation.game.id }}"
                >{{ getFirstGameTitle }}</router-link>
                ({{ this.recommendation.release.platform.name }})
                you may also like
                <router-link
                  :to="{name: 'Game', params: { id: this.recommendation.second_game.id }}"
                >{{ this.getSecondGameTitle }}</router-link>
                ({{ this.recommendation.second_release.platform.name }})."
              </h2>
              <hr>
              <div v-html="compiledMarkdown" class="content"></div>
              <div class="has-text-grey-light">
                Created On: {{ dateFormat(this.recommendation.created_at) }}
                <br>
...
export default {
  ...
  computed: {
    ...
    getFirstGameTitle() {
      var title = this.recommendation.release.alternate_title
        ? this.recommendation.release.alternate_title
        : this.recommendation.game.title;
      return title;
    },
    getSecondGameTitle() {
      var title = this.recommendation.second_release.alternate_title
        ? this.recommendation.second_release.alternate_title
        : this.recommendation.second_game.title;
      return title;
    }
  },
  methods: {
    ...
    updateRecommendation(recommendation) {
      ...
      NProgress.start();
      axios
        .patch("/api/recommendations/" + this.recommendation.id, {
        ...
        .catch(error => {
          if (
            ...
            flash("Recommendation Not Updated", "error");
          }
          NProgress.done();
          success = false;
        })
    ...
    deleteRecommendation() {
      ...
      NProgress.start();
      axios
        .delete("/api/recommendations/" + this.recommendation.id)
        .catch(error => {
          if (error.response.status === 403) {
            flash(error.response.data.error, "error");
          } else {
            flash("Unable to Remove Recommendation", "error");
          }
          NProgress.done();
          success = false;
        })
  ...
  created() {
    axios
      .get("/api/recommendations/" + this.$route.params.id)
      .then(response => {

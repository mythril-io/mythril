  ...
  <div>
    <div class="columns">
      <div class="column is-8 is-9-widescreen">
        ...
        <div v-if="this.orderedLibraryPlaying.length > 0">
          <div class="content">
                ...
                <b-tooltip :label="getLibraryTitle(library)">
                  <figure class="card image imageFade item-shadow">
                    <div class="text-container">
                      <img
                        v-lazy="$store.state.cdnURL + 'games/icons/' + library.game.icon"
          ...
          <div class="columns is-multiline is-mobile">
            <div
              class="column is-3-mobile is-3-tablet is-3-desktop is-2-widescreen"
              v-for="library in orderedLibraryContinuous"
              :key="library.id"
            >
              <router-link :to="{name: 'Game', params: { id: library.game.id }}">
                <b-tooltip :label="getLibraryTitle(library)">
      ...
      <div class="column">
        ...
        <div class="columns is-multiline is-mobile" v-if="displayedUser.favourites.length > 0">
          <div class="column is-3-mobile is-6-tablet" v-for="favourite in orderedFavourites">
            <router-link :to="{name: 'Game', params: { id: favourite.game.id }}">
              <b-tooltip
                :label="favourite.release.alternate_title ? favourite.release.alternate_title : favourite.game.title"
              >
                <figure class="card image imageFade item-shadow">
                  <div class="text-container">
                    <img
                      v-lazy="$store.state.cdnURL + 'games/icons/' + favourite.game.icon"
                      :alt="favourite.game.title"
                    >
        ...
        <article class="message is-warning" v-else>
          <div class="message-body">No favourites.</div>
        </article>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    details: function() {
      return (
        this.displayedUser.birthday ||
        this.displayedUser.location ||
        this.displayedUser.gender
      );
    },
    compiledMarkdown: function() {
      return md.render(this.displayedUser.about_me);
    },
    orderedLibraryPlaying: function() {
      var playingEntries = _.filter(this.displayedUser.libraries, [
        "play_status_id",
        1
      ]);
      return _.orderBy(playingEntries, "game_id", "desc");
	},
    orderedLibraryContinuous: function() {
      var playingEntries = _.filter(this.displayedUser.libraries, [
        "play_status_id",
        6
      ]);
  ...
  methods: {
    getLibraryTitle(libraryItem) {
      return libraryItem.release.alternate_title
        ? libraryItem.release.alternate_title

	    ...
	    <div class="container">
			...
			<h2 class="subtitle has-text-centered-desktop">
				<span class="has-text-grey-dark is-size-6">
					View the profiles of registered users
				</span>
			</h2>
	    </div>
	  </div>
			...
			<div v-else-if="users.length > 0">
				...
				<article class="message">
				  <div class="message-body">
				    Currently, there are <strong>{{ total }}</strong> registered users. <strong><router-link :to="{name: 'Register'}">Register Today!</router-link></strong>
				  </div>
				</article>
...
export default {
	...
	methods: {
		getUsers(page = 1) {
			...
			.then((response) => { 
				if(response.data.data.length == 0) { this.$router.push({ name: 'Users'}) }
